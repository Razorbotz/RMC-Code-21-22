...
void GLViewer::init(int w, int h, int nb_pc) {
	...
	for(auto& it : pointCloud_)
		it.initialize((int)res.width, (int)res.height, ctx);
...
void GLViewer::updatePointCloud(sl::Mat& matXYZRGBA, int id) {
	while(pointCloud_[id].lock) sl::sleep_ms(1);
	pointCloud_[id].pushNewPC(matXYZRGBA);
}

bool GLViewer::askReset() {
	auto ua = user_action;
	user_action = false;
	return ua;
}
...
void GLViewer::draw() {
	...
	for(auto& it : pointCloud_)
		it.draw(sl::Transform::transpose(vpMatrix));
}
...
PointCloud::PointCloud() : hasNewPCL_(false), initialized_(false), lock(false) {
}
